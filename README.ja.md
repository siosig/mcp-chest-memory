...
clone 不要 — 1 コマンドで導入が完了します:

```bash
npx -y -p mcp-chest-memory chest-memory-setup --yes
```

これだけで MCP サーバーの登録（実行は `npx -y mcp-chest-memory` 経由）、
`/chest-memory` スキルの配置、フックの設定まで完了します。データベース
スキーマは初回起動時に自動作成され、embedding モデル（約 120MB）は初回
利用時にバックグラウンドでダウンロードされます — モデル準備前の保存は
`pending` のまま記録され、準備でき次第自動で補完されます。

### ソースから導入（開発、LAN/WAN バックエンド構築）
...
Claude Code への MCP サーバー登録、`/chest-memory` スキルの配置、フックの設定。
...
記憶ストアに取り込み、embedding まで 1 コマンドで完結します:

```bash
npx -y -p mcp-chest-memory chest-memory-import --all
```

embedding モデル（約 120MB）は初回利用時に自動ダウンロードされます。
`--skip-embed` を渡すと embedding 補完をスキップします（バックグラウンド
メンテナンスが後から補完します）。`--dry-run` を渡すと書き込みなしで
パースと件数レポートのみ実行します。
...
- **セッション終了のたび**（フック、`install.sh` が設定）: Stop のたびに
...
- **スキル**: `/chest-memory`（`install.sh` が配置）が直前の会話を
  ...
  `/chest-memory status` でストアの状態を確認できます
- **フック**（`install.sh` が設定。npm インストール時は
  `chest-memory-setup --yes`）: `chest-memory-precompact` がコンテキスト
  圧縮前に作業状態スナップショットを保存、`chest-memory-session-start` が
  復元、`chest-memory-sync`（Stop フック）がセッションを自動キャプチャ。
  `node dist/bin/install-hooks.js` で再設定、`--remove` で解除できます

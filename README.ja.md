...
- [動作の仕組み](#動作の仕組み)
  ...
  - [ストレージ](#ストレージ)
  - [全文検索: FTS5 trigram](#全文検索-fts5-trigram)
...
- **スキル**: `/chest-memory`（`chest-memory-setup` が配置）が直前の会話を
  ...
  `/chest-memory status` でストアの状態を確認できます
- **フック**（`chest-memory-setup --yes` が設定）: 3 つのフックが
...
| フックイベント | コマンド | 動作 |
|---|---|---|
| `Stop` | `chest-memory-sync` | アシスタントターン終了時に発火。stdin から `transcript_path` を受け取り、セッションを記憶ストアにインポートします。 |
| `PreCompact` | `chest-memory-precompact` | Claude Code がコンテキストを圧縮する直前に発火。作業状態スナップショット（≤ 2 KB）を `session_snapshots` に保存（UPSERT）します。コンパクションは絶対にブロックしません。 |
| `SessionStart` | `chest-memory-session-start` | セッション開始時に発火。`source` が `compact` または `resume` の場合のみ、保存済みスナップショットを `<session_knowledge>` として新しいコンテキストに注入します。新規起動や clear では何も注入しません。 |

3 つのフックはすべてエラー発生時も `0` で終了（フェイルサイレント）し、
`~/.chest-memory/hook.log`（1 MB でローテート、owner-only `0600`）にログを記録します。
リモートモードでは `CHEST_MODE=remote`・`CHEST_REMOTE_URL`・`CHEST_API_TOKEN` を
コマンドに埋め込み、セッションデータをローカル SQLite ではなくバックエンドへ転送します。

...
- [診断と信頼性](#診断と信頼性)
  - [doctor server / doctor client](#doctor-server--doctor-client-1)
  - [fetch-model](#fetch-model-1)
  - [pending-resync](#pending-resync-1)
  - [破壊的変更: remote モードでのクライアント側 embed がデフォルト](#破壊的変更-remote-モードでのクライアント側-embed-がデフォルト)
- [Claude Code 連携](#claude-code-連携)
  ...
  - [ルール — chest-memory-rules](#ルール--chest-memory-rules)
- [開発](#開発)
- [セキュリティ](#セキュリティ)
  - [脅威モデル](#脅威モデル)
  - [原則](#原則)
...
[`deploy/nginx/nginx.conf.example`](deploy/nginx/nginx.conf.example) を nginx 設定に
...
remote モード（Docker）における登録・検索のデータフローを示します。
クライアントは Zod parse と HTTP 転送を担い、処理は原則 Docker バックエンドで実行されます。
**例外は登録時の embedding です**: バックエンドが `/capabilities` で
`server_has_embedder=false` を返す構成（`CHEST_SYNC_EMBED=0` + `CHEST_AUTO_MAINTENANCE=0`）では、
クライアントが bge-m3 で embed してベクトルを送ります（下図の点線）。既定では
`server_has_embedder=true` で、サーバーが embed します。

#### 登録フロー（chest_remember）

①②③ は HTTP レスポンス返却前に同期実行されます。④ のみ応答後の fire-and-forget です。
点線はクライアント側 embed 構成（`server_has_embedder=false`）でのみ通る経路です。

```mermaid
flowchart TD
    CC([Claude Code])

    subgraph CLIENT ["クライアント PC — stdio MCP サーバー"]
        MCP_W["RemoteExecutor\nZod parse → HTTP 転送"]
        CE["client-embed bridge\nserver_has_embedder=false の時のみ\nbge-m3 で embed"]
    end

    subgraph DOCKER ["Docker バックエンド — LocalExecutor\n①②③ は応答前に同期実行　④ のみ非同期"]
        V["① バリデーション・前処理\nresolveLayer / 長さ / ペースト検出\nredactText · Sudachi tokenize"]
        W["② 書き込み\nentity upsert\nINSERT memories (status=pending)\nINSERT events · supersedes archive\nrefreshMomentum"]
        E["③ Embedding（既定 = サーバー側）\nbge-m3 → status=done\nevaluateSupersessionFor\ncosine ≥ 0.97 → 旧記憶 archive"]
        EP["POST /memories/:id/embedding\nクライアント計算ベクトルを保存\n→ status=done"]
        M[["④ バックグラウンドメンテナンス\nvoid fire-and-forget · 10 分スロットル / flock\nrunActivationPhase — ACT-R 減衰計算\nrunDecayPhase — cold 圧縮 + TTL 失効\nrunSupersessPhase — 重複検出 archive\nrunLocalPendingSweep — pending embed 補完"]]
    ...
    MCP_W -.->|"capabilities で server_has_embedder=false の時"| CE
    CE -.->|"POST /memories/:id/embedding（vector + content_sha1）"| EP
    EP -.-> DB_W
```

#### 検索フロー（chest_recall）

すべて同期実行です。FTS 経路とベクトル経路は handleChestRecall 内で並行して走ります。
...
| 複数 PC（WAN） | stdio → nginx (TLS) → Docker | ホスト bind mount | 上記 + `deploy/nginx/nginx.conf.example` |
...
#### API ネゴシエーションと運用エンドポイント（v1.5+）

remote クライアントは `GET /capabilities` でサーバーと能力交渉します。

- `api_version` / `min_required_client_version`: 互換性チェック（古いクライアントは拒否）
- `server_has_embedder`: 登録時の embed をどちら側で行うかの決定（上記登録フロー参照）。
  `CHEST_SYNC_EMBED` / `CHEST_AUTO_MAINTENANCE` から導出され、`CHEST_MODE` には依存しません
  （バックエンドは常に local モード。`remote` は Prisma を無効化し DB を壊すため）。

REST バックエンドが公開する主なエンドポイント:

| メソッド・パス | 用途 |
|---|---|
| `POST /api/tools/:tool` | 8 つの MCP ツールを実行（local と同一の executor） |
| `GET /healthz` | ヘルスチェック（認証不要） |
| `GET /capabilities` | バージョン / 機能 / `server_has_embedder` のネゴシエーション |
| `GET /memories/pending` | embed 待ち行のページング取得（pending-resync 用） |
| `POST /memories/:id/embedding` | クライアント計算ベクトルの保存（dim / content_sha1 を検証） |
| `GET /diagnostics/db` | サーバー内 Prisma による DB 健全性（doctor server 用） |

運用 CLI（詳細は [診断と信頼性](#診断と信頼性)）:

- `chest-index doctor server` / `doctor client` — 構成診断（doctor server は `/diagnostics/db` と `/capabilities` を利用）
- `chest-index fetch-model` — bge-m3 / reranker のアトミック先読み
- `chest-index pending-resync` — pending 行をクライアント側で embed して回収
...
| `CHEST_CLIENT_EMBED` | `auto` | embedding をどこで計算するか。`auto` = `CHEST_MODE=remote` のときクライアント側、それ以外はサーバー側。`true`/`false` で強制。[破壊的変更の注記](#破壊的変更-remote-モードでのクライアント側-embed-がデフォルト)参照 |
...
設定の問題を診断し、embedding パイプラインを健全に保つための `chest-index`
サブコマンドが 4 つあります。いずれも `--json` で機械可読出力に切り替えられ、
終了コードの規約は共通です: **`0`** 全 check ok、**`1`** warn のみ、**`2`** 1 件以上 fail。

### doctor server / doctor client

`chest-index doctor server` はバックエンドのデプロイ構成を検査します（Docker
デーモン、コンテナの health、`compose.override.yaml` の適用、SQLite の integrity /
journal mode / 書き込み可否、`CHEST_API_TOKEN`、`/healthz` + `/capabilities` の疎通）。
`chest-index doctor client` はローカルのエージェント構成を検査します（MCP 登録、
`~/.claude/rules/mcp-chest-memory.md`、`/chest-memory` スキル、bge-m3 モデルキャッシュ、
設定済みリモートバックエンドへの疎通）。

SQLite 系のチェックはバックエンド内（`GET /diagnostics/db`）でライブ接続を使って
実行されるため、doctor を実行するホストに `node:sqlite`（Node 22.5+）も
コンテナのデータボリュームパスの知識も不要です。

fail した check には具体的な `fix:` ヒントが表示されます。

`doctor server` はトークン保護されたエンドポイント（`/capabilities`・`/diagnostics/db`）を
叩くため、実行環境に **サーバと同じ `CHEST_API_TOKEN` を設定**してください
（未設定だと該当チェックが `401` で fail します）。

```bash
CHEST_API_TOKEN=<token> chest-index doctor server   # 人間可読レポート
chest-index doctor server --json                    # CI / スクリプト用
chest-index doctor server --container chest-memory --timeout 5
chest-index doctor client
```

例（最も多いデプロイミス: compose override の付け忘れ）:
...
`chest-index fetch-model` は有効な embedding モデル（bge-m3）を、`--reranker`
指定時はクロスエンコーダも、ローカルキャッシュへ**アトミックに**先読みします。各
ファイルは `.tmp` パスに書いてから `fs.rename` で配置し、中断した実行が残した
部分・ゼロバイトファイルは次回起動時にパージします。これにより、`chest_remember`
がオンデマンドのダウンロードを誘発した際の初回書き込みタイムアウトを未然に防ぎます。
Docker イメージは起動時にこれを自動実行します。

```bash
chest-index fetch-model              # 未キャッシュなら bge-m3 をダウンロード
chest-index fetch-model --reranker   # reranker も取得
chest-index fetch-model --force      # キャッシュ済みでも再ダウンロード
chest-index fetch-model --model <hf-id>
```

### pending-resync

サーバーが embedder を持たない（remote モード）場合、記憶はクライアントが embed
するまで `embedding_status='pending'` で書き込まれます。`chest-index pending-resync`
はこのバックログを解消します: バックエンドから pending 行を一覧取得し、各行を
ローカルの bge-m3 で embed し、`POST /memories/:id/embedding` でベクトルを返送します。

```bash
chest-index pending-resync                       # pending を全件解消
chest-index pending-resync --dry-run             # バックログ件数のみ報告
chest-index pending-resync --batch-size 50 --concurrency 4 --max-retry 3
```

再実行は安全です: embed 済み行はスキップ、`409`（内容衝突）は再取得して 1 度だけ
リトライ、一時的な `5xx` は指数バックオフします。

### 破壊的変更: remote モードでのクライアント側 embed がデフォルト

本リリースより、remote モードのクライアントはデフォルトでローカル embed を行う
意思を持ちます（`CHEST_MODE=remote` のとき `CHEST_CLIENT_EMBED=auto` → `true`）。
クライアントが**実際に** embed するかはバックエンドの `/capabilities` で決まります:

- `server_has_embedder: true`（既定 — バックエンドが新規 memory を自分で embed）:
  クライアントはテキストを送り、**サーバーが embed** します。
- `server_has_embedder: false`: クライアントが**ローカルで embed** してベクトルを
  送るため、バックエンドは bge-m3 をロードしません（イメージ縮小・RSS 低下）。

バックエンドを embed しない構成（`server_has_embedder: false`）にするには
`CHEST_SYNC_EMBED=0` **かつ** `CHEST_AUTO_MAINTENANCE=0` を設定します（write時embed・
maintenance sweep の両方を停止）。`server_has_embedder` はこの2フラグから導出され、
**`CHEST_MODE` には依存しません**: REST バックエンドは常に local モードで動作する
（`remote` は DB を無効化する）ため、モードだけでは「サーバーは embed しない」を
表現できないからです。

**embed しないバックエンドでの必要な対応**: 各クライアントでモデルキャッシュを
用意してください — マシンごとに一度 `chest-index fetch-model` を実行します。
キャッシュが無いクライアントでは書き込みが `pending` 行を残すフォールバックになり、
後で `chest-index pending-resync` で回収できます。

クライアント側の挙動は `CHEST_CLIENT_EMBED=true|false` で明示的に強制できます。

## Claude Code 連携

> **この節は参照用です。** インストール手順（`chest-memory-setup --yes`）を
> 実行済みであれば、スキル・フック・ルールはすべて自動で設定されます。
> 動作しない場合の確認や、個別コンポーネントの再設定・削除の参考としてください。

`chest-memory-setup --yes` で次の設定が自動で行われます:

- **スキル**: `/chest-memory` が直前の会話を `realize` / `learning` に自動分類して保存し、
  判定根拠を表示します。`/chest-memory status` でストアの状態を確認できます
- **フック**: 4 つのフックが `~/.claude/settings.json` に登録されます。
  `npx -y -p mcp-chest-memory chest-memory-install-hooks` で再設定、`--remove` で解除できます
- **ルール**: `~/.claude/rules/mcp-chest-memory.md` がインストールされ、`chest_recall` /
  `chest_remember` を呼ぶべきタイミングをエージェントに伝えます。`applyTo: "**"` により
  全プロジェクトに自動適用されます
...
#### ルール — `chest-memory-rules`

`chest-memory-setup` のステップ 3/4 で、パッケージ内の `deploy/claude/rules/mcp-chest-memory.md` が
`~/.claude/rules/mcp-chest-memory.md` にコピーされます。Claude Code がセッション開始時に
このファイルを読み込み、エージェントに `chest_recall` / `chest_remember` を呼ぶべき
タイミング・レイヤー分類・禁止事項を伝えます。`applyTo: "**"` が指定されており、
すべてのプロジェクトに自動適用されます。

**更新**（パッケージ更新後に最新版を反映する場合）:

```bash
npx -y -p mcp-chest-memory chest-memory-setup --yes
```

`chest-memory-setup` は冪等です — 既存ファイルと内容が同一であればスキップし、
異なる場合のみ上書きします。
...
削除しても MCP サーバーやフックの動作には影響しません。`/chest-memory` スキルが
同等の指示を提供するため、スキルを使用する場合はルールが冗長になります。両方を残しても
問題はありません。
...
| コンテナ／ホスト侵害 | Docker イメージはバインドマウント上で非 root `node` ユーザとして実行。メンテナンスロックは world-writable な `/tmp` ではなくユーザ所有のデータディレクトリに置く。 | `deploy/docker/Dockerfile`, `src/cli/chest-index-flock.ts` |
| 脆弱な認証／ネットワーク露出 | バックエンドは 32 文字以上の Bearer トークンを要求し定数時間比較。`CHEST_BIND_HOST` で待受ホストを制御し、リクエストボディを 1 MB に制限（セッション取り込みエンドポイントは 50 MB）。nginx サンプルは HSTS と制限的 CSP を送出。 | `src/http/`, `deploy/nginx/nginx.conf.example` |

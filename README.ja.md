...
クライアントは Zod parse と HTTP 転送のみを担い、すべての処理は Docker バックエンドで実行されます。
...
```bash
chest-index doctor server                       # 人間可読レポート
...
本リリースより、`CHEST_MODE=remote` のときクライアントが embedding を**ローカルで**
計算するのがデフォルトになりました（`CHEST_CLIENT_EMBED=auto` → `true`）。ベクトルを
バックエンドへ送るため、バックエンドは bge-m3 を持たずに動作でき（イメージ縮小・
RSS 低下）、remote モードでのサーバー側 maintenance / embedding sweep は完全に
無効化されます。

**必要な対応**: remote モードの各クライアントでモデルキャッシュを用意してください
— クライアントマシンごとに一度 `chest-index fetch-model` を実行します。モデル
キャッシュが無いクライアントでは書き込みが `pending` 行を残すフォールバックになり、
後で `chest-index pending-resync` で回収できます。

以前のサーバー側 embed の挙動を維持したい場合は、クライアントで
`CHEST_CLIENT_EMBED=false` を設定し、`/capabilities` が `server_has_embedder: true`
を返すプロファイルでバックエンドを運用してください（共有バックエンドでは bge-m3 を
サーバー側に残し、フラグは `false` のままにします）。

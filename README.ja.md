...
2. [`deploy/nginx.conf.example`](deploy/nginx.conf.example) を nginx 設定に
   ...
   `nginx -t && systemctl reload nginx`。設定例はバックエンドを
   `/chest-memory` パスプレフィックス配下に公開します（nginx がプレフィックスを
   取り除いて転送するためバックエンド側は無変更）。死活監視は
   `https://chest.example.com/chest-memory/healthz` で行えます。
3. クライアントはプレフィックスを含む公開 URL で登録します:

```bash
./tools/install.sh --remote https://chest.example.com/chest-memory --token <トークン>
```

...
- **ファイル読み取り**: `chest_read_smart` は MCP クライアントが宣言した root 配下のみを
  読み取り、root を持たない REST バックエンドでは無効になる。トークン保有者がバックエンド
  ホストの任意ファイルを読むことはできない。
...
| リスク | 対策 | 該当箇所 |
|---|---|---|
| `chest_read_smart` 経由の任意ファイル読み取り | 読み取りを MCP クライアント宣言の root 配下に限定。チェック前に symlink を解決（`realpath`）し、`stat` と `read` で同一の正規化パスを使う（check/use の隙間なし）。空 root は全拒否なので、root を持たない REST バックエンドはツール自体を拒否する。 | `src/mcp/roots.ts`（`confinePath`）, `src/mcp/read-smart.ts` |
...
| 脆弱な認証／ネットワーク露出 | バックエンドは 32 文字以上の Bearer トークンを要求し定数時間比較。`CHEST_BIND_HOST` で待受ホストを制御し、リクエストボディを 1 MB に制限。nginx サンプルは HSTS と制限的 CSP を送出。 | `src/http/`, `deploy/nginx.conf.example` |
